...
const ShaderBackground = () => {
  ...
  const fsSource = `
    ...
    const vec4 lineColor = vec4(0.13, 0.77, 0.37, 0.6); // Lower alpha for lines
    ...
    void main() {
      ...
      vec4 lines = vec4(0.0);
      vec4 bgColor1 = vec4(0.05, 0.07, 0.05, 1.0); // Dark Greenish base
      vec4 bgColor2 = vec4(0.01, 0.02, 0.01, 1.0); // Deeper Greenish base

      for(int l = 0; l < 8; l++) { // Static loop count for WebGL 1.0 compatibility
        float normalizedLineIndex = float(l) / float(8);
      ...
      fragColor.a = 1.0;
      fragColor += lines * 0.6; // Further subdue the lines
  ...
  return (
    <canvas 
      ref={canvasRef} 
      className="fixed top-0 left-0 w-full h-full -z-10 brightness-50 contrast-125 opacity-70" 
